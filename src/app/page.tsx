    ...
    <div className="flex-1 flex flex-col max-w-lg mx-auto w-full p-4 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <form action={async () => { "use server"; await signOut({ redirectTo: "/login" }); }}>
          <button className="text-sm text-gray-400">Esci</button>
        </form>
      </div>

      <p className="text-gray-400">Ciao, {authSession.user.name || authSession.user.email}!</p>
      ...
      <div className="space-y-2">
        {sessions.map((s) => (
          <Link key={s.id} href={`/session/${s.id}`}
            className="block p-4 rounded-lg bg-gray-800 border border-gray-700">
      ...
      <Link href="/stats" className="block text-center p-3 rounded-lg bg-gray-800 border border-gray-700 font-semibold">
        📊 Statistiche
      </Link>
    </div>
  );
